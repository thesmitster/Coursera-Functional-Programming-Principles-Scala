...
trait Solver extends GameDef {
   ...
   */
  def from(initial: Stream[BlockState], explored: Set[Block]): Stream[BlockState] = ???
/*
    for {
      state ← initial
      neighbor ← newNeighborsOnly(neighborsWithHistory(state), explored)
    } yield from(neighbor #:: Stream.empty, explored + neighbor.block)
*/
  ...
  lazy val pathsFromStart: Stream[BlockState] = {
    ...
    val startStream: Stream[BlockState] = startState #:: Stream.empty
    from(startStream, Set())
  }
  ...
  lazy val solution: List[Move] =
    pathsToGoal.sortWith(_.history.length < _.history.length).head.history
}
